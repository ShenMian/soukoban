...
use crate::{direction::Direction, map::Map, point::Point, Tiles};
...
/// function.
pub fn find_path(from: Point, to: Point, can_move: impl Fn(Point) -> bool) -> Option<Vec<Point>> {
...
pub fn player_move_path(map: &Map, to: Point) -> Option<Vec<Direction>> {
...
fn convert_path_from_points_to_directions(path: Vec<Point>) -> Vec<Direction> {
    path.windows(2)
...
pub fn box_move_waypoints(
    ...
    let mut path: HashMap<(Point, Direction), u64> = HashMap::new();
...
pub fn construct_box_path(
    from: Point,
    to: Point,
    waypoints: &HashMap<(Point, Direction), u64>,
) -> Vec<Point> {
    let mut path = Vec::new();
    let mut current = to;
    // FIXME: 遇到回头路会提前退出
    while current != from {
        ...
        let mut min_neighbor = Point::zeros();
...
pub fn pushable_boxes(map: &Map) -> HashSet<Point> {
...
pub fn reachable_area(
    position: Point,
    can_move: impl Fn(Point) -> bool,
) -> HashSet<Point> {
    let mut reachable_area = HashSet::new();
    let mut deque = VecDeque::<Point>::new();
...
pub fn normalized_area(area: &HashSet<Point>) -> Option<Point> {
    area.iter()
