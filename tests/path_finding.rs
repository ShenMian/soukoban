use soukoban::{FxHashSet, Vector2, path_finding::*, prelude::*, solver::Strategy};
...
fn find_path() {
    ...
    assert_eq!(
        compute_player_move_directions(&map, Vector2::new(21, 35)),
        None
    );
}
...
fn test_compute_box_waypoints() {
    for strategy in [Strategy::PushOptimal, Strategy::MoveOptimal] {
        ...
        let (waypoints, _) = compute_box_waypoints(&map, Vector2::new(6, 2), strategy);
        let positions: FxHashSet<_> = waypoints.keys().map(|dp| dp.position).collect();
        assert_eq!(positions.len(), 15);

        let map = load_level_from_file("assets/Microban_II_135.xsb", 132).into();
        let (waypoints, _) = compute_box_waypoints(&map, Vector2::new(8, 7), strategy);
        let positions: FxHashSet<_> = waypoints.keys().map(|dp| dp.position).collect();
        ...
        let (waypoints, _) = compute_box_waypoints(&map, Vector2::new(18, 18), strategy);
        let positions: FxHashSet<_> = waypoints.keys().map(|dp| dp.position).collect();
        let box_path = construct_box_path(
        ...
        let (waypoints, _) = compute_box_waypoints(&map, Vector2::new(16, 34), strategy);
        let box_path = construct_box_path(
        ...
        let (waypoints, _) = compute_box_waypoints(&map, Vector2::new(21, 36), strategy);
        let box_path = construct_box_path(
